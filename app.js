...
function saveProgress() {
  try { localStorage.setItem('carnet_progress', JSON.stringify(State.progress)); State.storageBroken = false; }
  catch (e) { console.error('[Carnet] Écriture localStorage (carnet_progress) impossible :', e); State.storageBroken = true; }
}
function saveMeta() {
  try { localStorage.setItem('carnet_meta', JSON.stringify(State.meta)); State.storageBroken = false; }
  catch (e) { console.error('[Carnet] Écriture localStorage (carnet_meta) impossible :', e); State.storageBroken = true; }
}
...
// ---- Audio : fichiers individuels servis directement par GitHub Pages ----
// NOUVELLE SOLUTION (l'ancienne, à base de zips + JSZip décompressés dans le
// navigateur, ne fonctionnait pas de façon fiable : trop de complexité côté
// client, gros fichiers à parser en JS sur mobile, dépendance à JSZip).
// Maintenant chaque mp3 est un fichier statique individuel sous audio/, et
// audio-index.json ne contient plus que des chemins ("audio/42_abandon_mot.mp3").
// On laisse le navigateur (et son cache HTTP natif) gérer le téléchargement :
// pas de JSZip, pas d'extraction, pas de gestion manuelle de cache — un
// <audio> standard qui pointe sur l'URL du fichier suffit.
//
// Les fichiers eux-mêmes n'ont pas besoin d'être re-uploadés à la main : le
// workflow GitHub Actions .github/workflows/extract-audio.yml extrait
// automatiquement, côté serveur, tous les audio-zips/*.zip déjà présents
// dans le repo vers des fichiers individuels dans audio/.

function preloadAudioFor(id) {
  if (!State.audioIndex || !State.audioIndex.has(id)) return;
  const entry = State.audioIndex.get(id).mot;
  if (typeof entry === 'string') {
    // Simple prefetch : on laisse le cache HTTP du navigateur faire le travail.
    fetch(entry, { cache: 'force-cache' }).catch(() => {});
  }
}
...
async function playAudioFor(id, category) {
  ...
  try {
    let audio;
    if (State.audioManifestMode) {
      // entry est un chemin relatif, ex. "audio/42_abandon_mot.mp3" — doit
      // exister littéralement à cette URL sur GitHub Pages.
      console.log(`[Audio] id=${id}/${category} : lecture directe de "${entry}".`);
    ...
    audio.addEventListener('error', (e) => {
      ...
      console.error(`[Audio] id=${id}/${category} : l'élément <audio> n'a pas pu lire "${entry}" (code ${code}).`, e);
      hideCompanion();
    });
    await audio.play();
    ...
    console.error(`[Audio] id=${id}/${category} : échec de lecture.`, e);
    hideCompanion();
    return false;
  }
}
...
function buildCollocationExercise(key) {
  ...
  const pool = shuffle(State.collocations.filter(x => x.id !== id && x.text.split(' ').length >= 2));
  const fakes = [];
  const seen = new Set([item.text.toLowerCase()]);
  for (const other of pool) {
    ...
    const fakeKey = fake.toLowerCase();
    if (!seen.has(fakeKey)) { seen.add(fakeKey); fakes.push(fake); }
  }
  // BUG CORRIGÉ : quand pas assez de leurres plausibles pouvaient être
  // fabriqués, l'ancien code complétait avec "<bonne réponse> + up/out/on/
  // through" — la bonne réponse apparaissait donc en toutes lettres comme
  // PRÉFIXE de chaque mauvaise option, ce qui la rendait repérable sans
  // rien comprendre ("l'option qui n'a pas de petit mot collé au bout").
  // On complète maintenant avec d'autres vraies collocations, jamais avec
  // une variante du texte correct.
  while (fakes.length < 3 && pool.length) {
    const other = pool[Math.floor(Math.random() * pool.length)];
    const otherKey = other.text.toLowerCase();
    if (!seen.has(otherKey)) { seen.add(otherKey); fakes.push(other.text); }
  }
...
/* ---------- Session runner ---------- */
function startSession(bypassGoal) {
  const goal = State.meta.dailyNewGoal;
  const introducedToday = dayHistory().newCount;
  // "Continuer quand même" : au-delà de l'objectif du jour, on propose un lot
  // supplémentaire de mots plutôt qu'un blocage total jusqu'au lendemain.
  const remainingNew = bypassGoal ? 20 : Math.max(0, goal - introducedToday);
...
function renderDashboard() {
  ...
  const sessionCount = due + Math.max(0, goal - introducedToday);
  const ctaBlock = sessionCount > 0 ? `
      <div class="card cta-block">
        <div class="goal-line">${sessionCount} carte(s) dans la session du jour, tous modules mélangés</div>
        <button class="btn btn-primary btn-lg btn-block" onclick="startSession()">Commencer la session du jour</button>
      </div>` : `
      <div class="card cta-block">
        <div class="goal-line">Objectif du jour atteint (${introducedToday} nouveaux mots introduits) — les prochaines révisions arrivent demain, la répétition espacée a besoin de ce délai pour fonctionner.</div>
        <button class="btn btn-teal btn-lg btn-block" onclick="startSession(true)">Continuer quand même (mots en plus, au-delà de l'objectif)</button>
      </div>`;
...
function clozeHTML(ex) {
  // BUG CORRIGÉ : ".replace()" sans le flag global ne masquait que la
  // PREMIÈRE occurrence du mot dans la phrase. Si le mot apparaissait deux
  // fois, la deuxième restait visible en toutes lettres à côté du blanc —
  // la réponse "fuitait" littéralement dans la phrase. On masque maintenant
  // TOUTES les occurrences (flag "g" ajouté au "i" existant).
  const sentence = ex.sentence.replace(new RegExp('\\b' + ex.answer.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b', 'gi'), '<span class="blank">&nbsp;</span>');
  ...
  return `
    ...
    <div class="ex-body">
      ...
      <div id="clozeFr" class="fr-hint hidden">${ex.fr || ''}</div>
...
function submitCloze() {
  ...
  const frHint = document.getElementById('clozeFr');
  if (frHint) frHint.classList.remove('hidden');
...
function phraseClozeHTML(ex) {
  // Même correctif que clozeHTML : masquer TOUTES les occurrences (flag "g").
  const re = new RegExp(ex.answer.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi');
...
function connectorMcqHTML(ex) {
  return `
    ...
    <div class="ex-body">
      <div class="ex-prompt-sentence">${ex.item.example.en.replace(new RegExp(ex.item.connector.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), '<span class="blank">&nbsp;</span>')}</div>
      <div class="mcq-grid" id="mcq-grid">${ex.options.map((o, i) => `<button class="mcq-opt" onclick="answerMcqByIndex(this, ${i})">${o}</button>`).join('')}</div>
    </div>
  `;
}
...
function renderSummary() {
  ...
  // DEBUG TEMPORAIRE — à retirer une fois le bug de la flamme identifié.
  const debugLine = `<p class="page-sub" style="text-align:center;font-family:var(--mono);font-size:11px;opacity:.7">debug — streak:${State.meta.streak} lastActiveDay:${State.meta.lastActiveDay} today:${todayKey()} stockageOK:${!State.storageBroken}</p>`;
  const storageWarning = State.storageBroken ? `<p class="page-sub" style="text-align:center;color:var(--brick-deep)">⚠️ Le stockage local ne fonctionne pas sur cet appareil/navigateur en ce moment — ta progression ne sera pas conservée après fermeture.</p>` : '';
  screensEl().innerHTML = `
    <div class="screen">
