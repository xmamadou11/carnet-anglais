...
function loadZipPart(zipName) {
  if (ZipCache.has(zipName)) return ZipCache.get(zipName);
  const p = (async () => {
    const res = await fetch('audio-zips/' + zipName, { cache: 'force-cache' });
    if (!res.ok) throw new Error('zip fetch failed: ' + zipName);
...
async function playAudioFor(id, category) {
  ...
  if (!State.audioIndex || !State.audioIndex.has(id)) return false;
  ...
  try {
    ...
    audio.addEventListener('error', hideCompanion);
    await audio.play();
    return true;
  } catch (e) { console.warn('audio play failed', e); hideCompanion(); return false; }
}
...
function buildExercise(key, forceNewFlashcard) {
  const kind = State.registry.get(key).kind;
...
function buildGrammarExercise(key) {
  ...
  if (drill.type === 'mcq') {
    return { type: 'grammar_mcq', key, point, drill, options: shuffle(drill.options) };
  }
...
function buildCollocationExercise(key) {
  ...
  return { type: 'collocation_mcq', key, item, options, answer: item.text };
}
...
function nextExerciseInSession() {
...
function openGrammarDetail(ptId) {
...
function openPhraseDetail(prefix, id) {
...
function openWordDetail(id) {
  ...
  const narratedList = (title, category, items) => {
    ...
    const btn = hasAudio(id, category) ? `<button class="chip" onclick="playAudioFor(${id}, '${category}')">▶ Écouter</button>` : '';
    const rows = items.map((x, i) => `<div class="numbered-row"><span class="num-idx">${i + 1}</span><span>${x}</span></div>`).join('');
    return `<div class="detail-section">
      <div class="h" style="display:flex;align-items:center;justify-content:space-between">${title} ${btn}</div>
  ...
  const examplesBtn = hasAudio(id, 'examples') ? `<button class="chip" onclick="playAudioFor(${id}, 'examples')">▶ Écouter</button>` : '';
  const examplesBlock = examplesList.length ? `<div class="detail-section">
      <div class="h" style="display:flex;align-items:center;justify-content:space-between">Exemples ${examplesBtn}</div>
  ...
  const defBtn = hasAudio(id, 'definition') ? `<button class="chip" onclick="playAudioFor(${id}, 'definition')">▶ Écouter</button>` : '';
  ...
  overlay.innerHTML = `
    <div class="detail-sheet">
      ...
      <div style="display:flex;align-items:center;gap:14px;margin-top:14px">
        <button class="audio-btn" ${audioAvailable ? '' : 'disabled'} onclick="playAudioFor(${id}, 'mot')">▶</button>
        <div class="translation-line">${w.translation}</div>
      </div>
      <div class="def-line" style="display:flex;align-items:center;justify-content:space-between;gap:10px">
        <span>${w.definition}</span>${defBtn}
      </div>
      ${examplesBlock}
...
function flashcardHTML(ex) {
  const w = ex.word;
  const audioAvailable = hasAudio(ex.id);
  return `
    <div class="ex-kind">Carte &middot; <span class="level-tag lvl-${w.level}">${w.level}</span></div>
    <div class="ex-body">
      <div><div class="ex-prompt-word">${w.word}</div><div class="ex-prompt-phon">${w.phonetic || ''}</div></div>
      <button class="big-audio-btn" ${audioAvailable ? '' : 'disabled'} onclick="playAudioFor(${ex.id})">♪</button>
      <div id="fc-reveal" class="hidden">
        <div class="translation-line">${w.translation}</div>
        <div class="def-line">${w.definition}</div>
        ${w.examples && w.examples[0] ? `<div class="example-line" style="border-top:none"><div class="en">${w.examples[0].en}</div><div class="fr">${w.examples[0].fr}</div></div>` : ''}
      </div>
...
function mcqHTML(ex, question, promptWord, phon) {
  const w = ex.word;
  const audioAvailable = w && hasAudio(ex.id) && ex.type === 'mcq_trans';
  return `
    <div class="ex-kind">${question}</div>
    <div class="ex-body">
      <div><div class="ex-prompt-word">${promptWord}</div>${phon ? `<div class="ex-prompt-phon">${phon}</div>` : ''}</div>
      ${audioAvailable ? `<button class="big-audio-btn" onclick="playAudioFor(${ex.id})">♪</button>` : ''}
      <div class="mcq-grid" id="mcq-grid">${ex.options.map(o => `<button class="mcq-opt" onclick="answerMcq(this, '${String(o).replace(/'/g, "\\'")}')">${o}</button>`).join('')}</div>
    </div>
  `;
}
...
function clozeHTML(ex) {
  ...
  return `
    ...
    <div class="ex-body">
      ${canListen ? `<button class="big-audio-btn" onclick="playAudioFor(${ex.id}, 'examples')">♪</button>` : ''}
...
function listeningHTML(ex) {
  return `
    ...
    <div class="ex-body">
      <button class="big-audio-btn" onclick="playAudioFor(${ex.id})">♪</button>
      <div style="font-size:13px;color:var(--ink-soft)">Écoute et écris le mot entendu.</div>
...
function connectorMcqHTML(ex) {
  return `
    ...
    <div class="ex-body">
      ...
      <div class="mcq-grid" id="mcq-grid">${ex.options.map(o => `<button class="mcq-opt" onclick="answerMcq(this, '${o}')">${o}</button>`).join('')}</div>
    </div>
  `;
}
...
function collocationMcqHTML(ex) {
  return `
    ...
    <div class="ex-body">
      <div class="mcq-grid" id="mcq-grid">${ex.options.map(o => `<button class="mcq-opt" onclick="answerMcq(this, '${o.replace(/'/g, "\\'")}')">${o}</button>`).join('')}</div>
    </div>
  `;
}
