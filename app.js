...
async function playAudioFor(id, category) {
  ...
  try {
    ...
    audio.addEventListener('error', (e) => { console.error(`[Audio] id=${id}/${category} : l'élément <audio> n'a pas pu lire "${entry}" (fichier absent à cette URL, ou corrompu). Vérifie que le fichier existe bien sur GitHub Pages à ce chemin exact.`, e); hideCompanion(); });
    await audio.play();
